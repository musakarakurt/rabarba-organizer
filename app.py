...
def contains_target_guest(description, episode_number):
    ...
    # 322'den önceki bölümler için filtreleme YAPMA - hepsini al
...
def load_episodes():
    ...
    try:
        ...
        offset = 0
        limit = 50
        max_pages = 48  # 2389 bölüm için 48 sayfa
        
        for page in range(max_pages):
            print(f"Sayfa {page + 1} yükleniyor (offset: {offset})...")
            ...
            print(f"Sayfa {page + 1}: {page_episodes} bölüm")
        ...
        # Filtreleme testi - 731. bölümü kontrol et
        episode_731 = next((ep for ep in sorted_episodes if ep['episode_number'] == 731), None)
        if episode_731:
            print(f"731. bölüm testi: {episode_731['name']}")
            print(f"731. bölüm açıklama: {episode_731['description']}")
            print(f"731. bölüm filtrelendi mi: {contains_target_guest(episode_731['description'], 731)}")
...
def view_lists():
    ...
    page_size = 100  # 100 bölüm sayfa başına
